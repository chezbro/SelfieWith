class ForgotPasswordUsernameController < UIViewController
  ...
  def next_action
    if @username.text.length > 0 and @phone_number.text.length > 0
      ...
      API.post("auth/reset_token", params) do |result|
        if result
          p result
          rmq.animations.stop_spinner
          self.navigationController.pushViewController(ForgotPasswordResetController.new(user: result["user"]), animated:true)
        else
          rmq.animations.stop_spinner
        end
      end
