class JoinSMSController < UIViewController
  ...
  def resent_confirmation_code
    ...
    AFMotion::SessionClient.shared.post("auth/resend", params) {}
  ...
  def next_action
    ...
    if @sms_code.text.length > 0
      ...
      AFMotion::SessionClient.shared.post("auth/confirm", params) do |result|
        ...
        if result.success?
          ...
          SimpleSI.alert("Please check with your confirmation code, or try to send new one.")
          Motion::Blitz.dismiss
        end
      end
    else
