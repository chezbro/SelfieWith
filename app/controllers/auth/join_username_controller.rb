class JoinUsernameController < UIViewController
  ...
  def scaleImage(image)
    if image && image.size.width < 512.0
      newimg = image
    else
  ...
  def next_action
    ...
    if @username.text.length > 0 and @password.text.length > 0 and @email.text.length > 0
      if rmq.validation.valid?(@username.text, :length, min_length: 4)
        if rmq.validation.valid?(@password.text, :length, min_length: 8)
          if rmq.validation.valid?(@email.text, :email)
            ...
            AFMotion::SessionClient.shared.multipart_post("auth/register", params) do |result, form_data|
          ...
          SimpleSI.alert({
            ...
            message: "You need a strong password, which at least 8 characters.",
            transition: "bounce",
            buttons: [
              {title: "Got it", type: "cancel"} # action is secondary
            ]
